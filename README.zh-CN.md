...
一个本地 stdio MCP relay，让 Codex 通过 `grok_delegate` 和 `cursor_delegate` 把任务交给 Grok 或 Cursor。每次调用都会启动独立的 agent 子进程，通过共享 ACP v1 runner 完成认证、创建或恢复会话以及一次 prompt。
...
请填写实际 Cursor CLI 的路径，不限文件名。不要假定 `agent` 就是 Cursor，Grok 也可能使用该命令名。未配置 Cursor 时仍注册两个工具，仅 Cursor 调用返回配置错误。委派前单独执行 Cursor 的 `login`，或通过 relay 环境提供 `CURSOR_API_KEY` / `CURSOR_AUTH_TOKEN`；不要把凭据放入工具参数。

可选：将两个工具的推荐委派说明添加到 Codex 全局指令中：

```bash
pnpm setup:codex-instructions
```

脚本会更新 `$CODEX_HOME/AGENTS.md`；未设置 `CODEX_HOME` 时则更新 `~/.codex/AGENTS.md`。它保留其他指令，将现有 `GROK-BUILD` 受管理块替换为两个 agent 的说明，不会重复插入。如果同一目录存在非空的 `AGENTS.override.md`，Codex 会改为加载该文件，脚本也会输出警告。
...
relay 注册两个 MCP 工具：`grok_delegate` 和 `cursor_delegate`。
...
Cursor 额外接受 `model?: string` 和 `mode?: "agent" | "ask"`，不开放 Plan 模式。Grok 输入、输出契约保持不变。
...
- Grok 元数据保留现有格式和路径。Cursor 元数据使用独立命名空间，并记录启动时的模型选择和模式。记录使用私有权限和哈希会话文件名。
- 实际对话历史由各后端保存；恢复时 relay 读取元数据并调用对应后端的 `session/load`。Cursor 和 Grok 会话 ID 不能互用。
- 未知 ID、损坏记录、不同 `cwd`、缺少 load 能力或 load 失败都会明确报错，不会静默创建替代会话。

### 并发与清理

- 同一规范化 `cwd` 使用两个后端和所有 relay 进程共用的排他锁；不同 worktree 可以并行。
...
上述 Grok sandbox 细节不代表 Cursor 的 sandbox 行为。Codex 应在委派前选择合适的 worktree，委派后检查实际 diff、运行相关测试并完成最终 review。不要仅根据 worker 的自然语言总结判断修改是否符合预期。
...
不要把本 relay 配置为下游 Cursor 或 Grok 的 MCP server。特别是 Cursor 会读取项目和用户 `.cursor/mcp.json`；ACP 的空 `mcpServers` 列表并不禁用这些配置。worker 会继承内部标记 `GROK_RELAY_DELEGATED=1`，在该标记环境下调用 relay 会被拒绝。这只是基于环境继承的循环保护，不是隔离机制；清除标记的包装脚本会使保护失效。
...
常规 `pnpm test` 跳过两个真实集成测试套件。自动化测试覆盖会话创建与恢复、后端隔离、模型/模式绑定、认证、cwd 锁、超时与取消、进程组清理、有界输出、进度、Cursor 扩展和权限拒绝，以及 Grok 异常权限行为。真实测试不能证明完整 sandbox 隔离，跳过的联调场景应单独说明。

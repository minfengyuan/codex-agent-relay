...
`codex-agent-relay` 连接 Codex 与支持 ACP 的 Coding Agent。Codex 负责统一编排和最终审查，relay 负责委派任务，并返回执行结果和可继续使用的会话 ID。
...
- **Codex 仍是总控** —— diff 审查、测试验证和最终集成都由 Codex 完成。
...
| OpenCode | `opencode_delegate` | 支持可选 model / effort / agent 配置的编码任务 |
| DSH | `dsh_delegate` | 通过 DeepSeek Harness 执行编码任务 |
...
Grok 默认使用 `grok`，OpenCode 默认使用 `opencode`。Cursor 需要显式配置可执行文件：
...
如有需要，可以通过 `CODEX_AGENT_RELAY_GROK_COMMAND` 和 `CODEX_AGENT_RELAY_OPENCODE_COMMAND` 覆盖对应可执行文件路径。
...
该命令会把推荐的 Agent 委派说明加入 Codex 指令，同时保留其他已有内容。

## 使用方式

配置完成后，可以直接让 Codex 用自然语言进行委派：
...
如果希望并行处理相互独立的任务，建议为它们使用不同 worktree。同一个工作目录上的委派会被串行化，避免并发修改。
...
- relay 会按进程树清理委派 worker；如果无法确认清理完成，会继续保留 workspace 锁，而不是假设环境已经安全。
...
真实 Agent 的 smoke test 可参考 `package.json` 中的对应脚本。

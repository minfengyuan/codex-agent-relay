...
Successful results include `provider: "dsh"` plus bounded `text`, optional `toolCalls` / `usage`, and `summariesTruncated` when summaries were capped. Before reporting success, the relay requires DSH to advertise `session/close` and waits for that request to complete so session updates and persistence are finalized. A missing capability, close timeout, or close failure returns `SESSION_CLOSE_UNSUPPORTED`, `SESSION_CLOSE_TIMEOUT`, or `SESSION_CLOSE_FAILED`; output already produced is retained in the partial result. Permission requests are refused (`reject_once` when offered) and abort with `PERMISSION_REQUIRED`.
...
- DSH success requires an acknowledged `session/close`. The relay still performs and verifies its normal process-tree cleanup afterward; it does not treat ACP session close as process cleanup.

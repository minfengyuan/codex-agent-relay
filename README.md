...
| `reasoningEffort` | no | Opaque advertised `reasoning_effort` session option; `""` is valid when advertised |

When both `model` and `reasoningEffort` are set, the relay applies `model` first, then `reasoning_effort`, using the option values advertised by the current session. Omitting either field leaves the CLI's existing configuration unchanged. When `reasoning_effort` advertises `""` as the provider default, pass `reasoningEffort: ""` to select it; whitespace-only values remain invalid. Invalid advertised values fail closed (`INVALID_CONFIG` / `CONFIG_UNSUPPORTED`). For a new DSH session, the relay saves the session binding before configuration; configuration failures therefore include a resumable `sessionId` in the partial result, which can be retried with the same `cwd`. A session ID is never exposed until its relay record is confirmed.

Successful results include `provider: "dsh"` plus bounded `text`, optional `toolCalls` / `usage`, and `summariesTruncated` when summaries were capped. Before creating or resuming a session, the relay requires DSH to advertise `session/close`; before reporting success, it waits for that request to complete so session updates and persistence are finalized. A missing capability, close timeout, or close failure returns `SESSION_CLOSE_UNSUPPORTED`, `SESSION_CLOSE_TIMEOUT`, or `SESSION_CLOSE_FAILED`; output already produced is retained in the partial result. For an active session that fails, the relay attempts close before process-tree cleanup. Cancellation, timeout, and permission failure send `session/cancel` first. An abnormal close failure is appended as bounded diagnostic context without replacing the original error. Permission requests are refused (`reject_once` when offered) and abort with `PERMISSION_REQUIRED`.
...
- DSH success requires an acknowledged `session/close`. Active failed sessions are also closed best-effort before process cleanup; cancellation, timeout, and permission failure send `session/cancel` first. The relay still performs and verifies process-tree cleanup afterward and does not let an abnormal close failure replace the original task error.

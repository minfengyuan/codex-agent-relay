...
describe("loadConfig OpenCode command", () => {
  it("defaults to opencode and trims an explicit command", () => {
    expect(loadConfig({}).opencodeCommand).toBe("opencode");
    expect(loadConfig({ GROK_RELAY_OPENCODE_COMMAND: "  /bin/oc  " }).opencodeCommand).toBe("/bin/oc");
    expect(loadConfig({ GROK_RELAY_OPENCODE_COMMAND: "   " }).opencodeCommand).toBe("opencode");
  });
});
...
describe("OpenCodeRunner", () => {
  ...
  it("rejects invalid or unsupported options and config timeouts without prompting", async () => {
    ...
    await expect(new OpenCodeRunner(config(state, { phaseTimeoutMs: 80 }), new SessionStore(state, "opencode")).delegate({
      task: "one",
      cwd,
      model: "opencode/gpt",
  ...
  it("cancels a hung prompt and kills leftover descendants", async () => {
    ...
    vi.stubEnv("FAKE_ACP_MODE", "hang");
    await expect(new OpenCodeRunner(config(state, { totalTimeoutMs: 150 }), new SessionStore(state, "opencode"))
